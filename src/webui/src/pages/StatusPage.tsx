...
export default function StatusPage({ status, onRefresh }: StatusPageProps) {
    ...
    const enabledGroups = Object.values(config.groupConfigs || {}).filter((group) => group.enabled).length
        ...
        <div className="space-y-6">
            ...
            <div className="card p-5 hover-lift animate-fade-in-up">
                ...
                <div className="space-y-3">
                    <InfoRow label="词库数量" value={`${status.dictionarySize} 条`} />
                    <InfoRow label="累计检查" value={`${stats.processed} 条`} />
                    <InfoRow label="已上报" value={`${stats.reported} 次`} />
                    <InfoRow label="已禁言" value={`${stats.banned} 次`} />
                    <InfoRow label="配置群数" value={`${enabledGroups} 个`} />
                    <InfoRow label="违规触发禁言" value={`${config.maxViolations} 次`} />
                    <InfoRow label="禁言时长" value={`${config.banDurationSeconds} 秒`} />
                    <InfoRow label="模型超时" value={`${config.guardTimeoutMs} 毫秒`} />
                </div>
            </div>
        </div>
    )
